...
describe('/api/operator', () => {
  ...
  describe('GET /', () => {
    it('should return all operators', async () => {
      ...
      const response = await request(server).get('/api/operator');
  ...
  describe('GET /:slug', () => {
    it('should return an operator if a valid slug is passed', async () => {
      ...
      const response = await request(server).get(`/api/operator/${operator.slug}`);
      expect(response.status).toBe(200);
      // expect(response.)
    });

    it('should return a 404 if invalid slug is passed', async () => {
      const response = await request(server).get('/api/operator/12');
  ...
  describe('DELETE /:id', () => {
    it('should delete the shop with the given ID from the database if JWT is an Admin', async () => {
      ...
      const response = await request(server)
        .delete(`/api/operator/${operator._id}`)
        .set('Authorization', `Bearer: ${token}`);
